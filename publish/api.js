...
const DATA_QUREIES_TABLE_NAME = 'DataQueries';
...
lib.Validator;
lib.ValidatorResult;
lib.ValidatorResultError;
lib.ValidationError;
lib.SchemaError;
lib.SchemaScanResult;
lib.scan;
var lib_8 = lib.validate;
...
class QueryService {
    ...
    async upsert(client, request) {
        const adal = this.papiClient.addons.data.uuid(config.AddonUUID).table(DATA_QUREIES_TABLE_NAME);
        const body = request.body;
        const validation = lib_8(body, QueriesScheme, { allowUnknownAttributes: false, });
        if (!validation.valid) {
            throw new Error(validation.toString());
        }
        if (!body.Key) {
            body.Key = C__Users_hadar_l_Documents_New_Framwork_Hadar_Tests_dataQueries_serverSide_node_modules_uuid.v4();
        }
...
async function queries(client, request) {
