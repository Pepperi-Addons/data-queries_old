...
export class AddonComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.addonService.getAddonApiCall(this.pluginService.addonUUID, 'api', 'queries').toPromise().then((dataQueries) => {
            dataQueries.forEach(dataQuerie => {
                this.queriesOptions.push({ key: dataQuerie.Key, value: dataQuerie.Key })
            });
        });
    ...
    onDataQuerySelected(event: IPepFieldValueChangeEvent) {
        this.addonService.postAddonApiCall(this.pluginService.addonUUID, 'elastic', `execute?query_id=${event}`).toPromise().then((dataQueries) => {
            this.importChartFileAndExecute(dataQueries);
        });
    }
    ...
    private importChartFileAndExecute(data) {
